...
Node* MoveToFront(Node** Head, double target) {
    ...
    while (Current != NULL) {
        if (Current->Data.score == target) {
            if (Current == (*Head)) {   // 내가 찾는 값을 가진노드가 헤드노드

            }
            else if (Current->NextNode == NULL) {   // 내가 찾는 값을 가진 노드가 꼬리노드인 경우 
               // Current노드를 링크에서 제외한다.
            ...
            else { // 내가 찾는 값을 가진 노드가 헤드하고 테일 사이에 있는 경우
               // Current 노드를 링크에서 제외한다.
                Current->PrevNode->NextNode = Current->NextNode;
...
// 계수법
Node* Gaesoo(Node** Head, double target)
{
    return NULL;
}
...
int main( void )
{
    ...
    while (1) {
        printf("찾는 값을 입력하세요(exit:-1):");
        scanf("%lf", &inputValue);
        if (inputValue < 0.0) break;
        system("cls"); //전체 화면 지우기
        Node* searchNode = SequentialSearch(List, inputValue);
